...
class PCSRX(LiteXModule):
    def __init__(self, lsb_first=False):
        ...
        # --------
        count = Signal() # Byte counter for config register.
        input_valid = self.decoder.valid

        # SGMII Timer.
        # ------------
        self.timer = timer = PCSSGMIITimer(speed=self.sgmii_speed, valid=input_valid)

        # Buffer.
        # -------
        self.buffer = buffer = stream.Buffer([("data", 8)], pipe_valid=True, pipe_ready=False)
        self.comb += If(timer.done & input_valid,
