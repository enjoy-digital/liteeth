...
class PMA_A7_GTP_5G_BASER(LiteXModule):
    ...
    top. What differs is underneath. The GTP's 64b/66b gearbox has a 32-bit user interface, so each
    block crosses it as two words: the one presented with the header carries block bits [63:32],
    the next carries [31:0]. The fabric-owned TXSEQUENCE counter still runs 0..32 over 33 TXUSRCLK2
    cycles, with the pause at 31 (UG482 Table 3-10), so 33 cycles carry 16 blocks. On receive,
    RXDATAVALID marks the words and RXHEADERVALID marks the first of each pair. tx_ce/rx_ce are
    therefore high once per block, never on consecutive cycles, and the user clock runs at
    linerate/32 while blocks move at linerate/66.
    """
    linerate    = 5.15625e9
    tx_clk_freq = linerate/32
    rx_clk_freq = linerate/32

    def __init__(self, qpll_channel, data_pads, sys_clk_freq, refclk_freq, tx_polarity=0,
        rx_polarity=0):
        ...
        # Receive restart request from the PCS (its serdes_rx_reset_req), in eth_rx.
        self.rx_reset_req = Signal()

        # A full reset: both init FSMs, the QPLL channel, and the transceiver.
        self.reset    = Signal()
        self.loopback = Signal(3)

        # Polarity is a port on the GTP rather than a parameter, so it can be changed at run time.
        # The arguments set their reset values; a design may drive them instead.
        ...
        # Status, named as in PMA_K7_GTX_10G_BASER.
        ...
        # Transceiver signals. Names match the ported GTPE2_CHANNEL parameter block.
        tx_reset          = Signal()
        tx_cm_locked      = Signal()
        ...
        # TX gearbox sequencing -------------------------------------------------------------------
        # Held at 0 until TXRESETDONE, the start-up order UG482 asks for, then 0..32 with the pause
        # at 31. A block is handed over as two words, so the PCS advances on the second word of each
        # pair: odd sequence values, plus 32, which carries the second word of the block that
        # straddles the pause.
        tx_gearbox_start = Signal()
        tx_word_ready    = Signal()
        tx_second_word   = Signal()
        self.specials += MultiReg(tx_reset_done, tx_gearbox_start, "eth_tx")
        self.sync.eth_tx += [
            If(~tx_gearbox_start,
                tx_sequence.eq(0),
            ).Elif(tx_sequence == 32,
        ...
        self.comb += [
            tx_word_ready.eq(tx_sequence != 31),
            tx_second_word.eq(tx_sequence[0] | (tx_sequence == 32)),
            self.tx_ce.eq(tx_gearbox_start & tx_word_ready & tx_second_word),
            # The PCS output only changes after tx_ce, so it holds the same block across both
            # words: bits [63:32] with the header, then [31:0]. Loopback cannot check this order,
            # since any self-consistent one works; a real peer can.
            tx_data.eq(Mux(tx_second_word, self.tx_data[:32], self.tx_data[32:])),
            tx_header.eq(Cat(self.tx_header, 0)),
        ]

        # RX gearbox reassembly --------------------------------------------------------------------
        # Keep the header word, and present the block on the word that follows it. The GT's outputs
        # are used as they arrive: a register stage in front of this cost 36 registers and, measured
        # on the Acorn, left receive timing worse rather than better (eth_rx slack +0.31ns with it,
        # +0.49ns without).
        rx_data_high    = Signal(32)
        rx_header_latch = Signal(2)
        rx_have_high    = Signal()
        self.sync.eth_rx += [
            If(rx_data_valid_gt[0],
                If(rx_header_valid_gt,
                    rx_data_high.eq(rx_data_gt),
                    rx_header_latch.eq(rx_header_gt[:2]),
                    rx_have_high.eq(1),
                ).Elif(rx_have_high,
                    rx_have_high.eq(0),
                ),
        ...
        self.comb += [
            self.rx_ce.eq(rx_data_valid_gt[0] & ~rx_header_valid_gt & rx_have_high),
            self.rx_data.eq(Cat(rx_data_gt, rx_data_high)),
            self.rx_header.eq(rx_header_latch),
        ]

        # UG482: RXGEARBOXSLIP must be pulsed for one RXUSRCLK2 cycle and then held low for at
        # least 32 cycles while the gearbox re-aligns. Rate-limit the PCS's requests accordingly.
        SLIP_HOLDOFF = 64
        rx_slip_d    = Signal()
        ...
        self.sync.eth_rx += [
            ...
            rx_slip_pulse.eq((rx_slip_wait == 0) & self.rx_slip & ~rx_slip_d),
        ]

        # Exposed for probing.
        self.tx_sequence     = tx_sequence
        self.gtp_tx_data     = tx_data
        self.rx_data_valid   = rx_data_valid_gt
        self.rx_header_valid = rx_header_valid_gt
        self.rx_slip_pulse   = rx_slip_pulse

        # GTPE2_CHANNEL ------------------------------------------------------------------------------
        clk25_div = ceil(refclk_freq/25e6)
        ...
        gtp_params = dict(
            ...
            p_RXCDR_CFG                  = {
                ...
                5.15625e9 : 0x0001107FE206021041010,
            }[self.linerate],
            ...
            # RX Fabric Clock Output Control Attributes
            p_RXOUT_DIV                  = {1.25e9 : 4, 3.125e9 : 2, 5.15625e9 : 1}[self.linerate],

            # TX Fabric Clock Output Control Attributes
            p_TXOUT_DIV                  = {1.25e9 : 4, 3.125e9 : 2, 5.15625e9 : 1}[self.linerate],
        ...
        gtp_params.update(
            ...
            o_RXDATAVALID          = rx_data_valid_gt,
            o_RXHEADER             = rx_header_gt,
            o_RXHEADERVALID        = rx_header_valid_gt,
            o_RXSTARTOFSEQ         = Open(),
        ...
        # Clocking ------------------------------------------------------------------------------------
        # UG482: in 4-byte mode xxUSRCLK2 = xxUSRCLK/2, with xxUSRCLK2 = linerate/32 so that a
        # 32-bit word carries the full line rate. TX/RXOUTCLK run at linerate/16, so one MMCM per
        # direction provides both, phase aligned.
        txoutclk_bufg = Signal()
        rxoutclk_bufg = Signal()
        self.specials += [
            Instance("BUFG", i_I=self.txoutclk, o_O=txoutclk_bufg),
            Instance("BUFG", i_I=self.rxoutclk, o_O=rxoutclk_bufg),
        ]

        self.tx_cm = tx_cm = S7MMCM()
        tx_cm.register_clkin(txoutclk_bufg, self.tx_clk_freq*2)
        ...
        self.comb += [
            tx_cm.reset.eq(tx_cm_reset),
            tx_cm_locked.eq(tx_cm.locked),
            self.tx_mmcm_locked.eq(tx_cm.locked),
        ]

        self.rx_cm = rx_cm = S7MMCM()
        rx_cm.register_clkin(rxoutclk_bufg, self.rx_clk_freq*2)
        ...
        self.comb += [
            rx_cm.reset.eq(rx_cm_reset),
            rx_cm_locked.eq(rx_cm.locked),
            self.rx_mmcm_locked.eq(rx_cm.locked),
        ]

        # Transceiver init ----------------------------------------------------------------------------
        # GTPTxInit resets the QPLL channel and holds GTTXRESET until it locks. Both init FSMs are
        # wrapped so that self.reset re-runs the whole sequence, QPLL included, as a full reset of
        # the Kintex-7 PMA does.
        self.tx_init = tx_init = ResetInserter()(GTPTxInit(sys_clk_freq))
        self.comb += [
        ...
        # GTPRxInit carries the production-silicon RX reset workaround (AR 53561), over DRP.
        self.rx_init = rx_init = ResetInserter()(GTPRxInit(sys_clk_freq))
        self.comb += [
            rx_init.reset.eq(self.reset),
        ...
        # Hold the transmit MMCM through GTTXRESET, so it only locks on a TXOUTCLK the transceiver
        # is actually producing.
        self.sync += tx_cm_reset.eq(~qpll_channel.lock | tx_reset)
        tx_cm_reset.attr.add("no_retiming")

        # CDR lock time of 50,000 UI, as DS183 and the Xilinx wizards assume.
